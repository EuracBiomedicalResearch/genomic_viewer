...
{
  libs <- c("shiny", "bslib", "shinyjs", "svglite", "svgPanZoom", "paletteer",
            "shinycssloaders", "ggplot2", "ComplexUpset", "dplyr", "readr",
            "stringr", "sqldf", "ggpubr", "circlize", "ggraph", "igraph",
            "org.Hs.eg.db", "AnnotationHub", "ggchicklet", "plotgardener",
            "spiky", "TxDb.Hsapiens.UCSC.T2T.knownGene",
            "TxDb.Hsapiens.UCSC.hg38.knownGene",
            "TxDb.Hsapiens.UCSC.hg19.knownGene",
            "TxDb.Mmusculus.UCSC.mm39.knownGene",
            "TxDb.Mmusculus.UCSC.mm10.knownGene",
            "rtracklayer", "ChIPpeakAnno" )
...
#' Load configuration file, depending on development or container version.
#' @returns A list as generated by config::get().
loadConfig <- function()
{
  if( in.container() ) {
  ...
  # throw an error when checking for missing keys below.) This is implemented
  # for any key defined in the container internal config file, and applies
  # specifically to `chrom.cen.dir` and `genes.hgnc.dir`.
  if( in.container() )
...
#' Does configuration contain all necessary entries?
#'
#' @param config Config file parameters (list).
#' @returns `TRUE`, yes, all is complete; `FALSE`, some config entries are
#' missing, they have been written to stderr.
configComplete <- function(config)
{
  ...
  if (length(missing.keys) > 0) {
    cat(file = stderr(), "File `GenomicViewer_config.yml`: Missing key(s): ",
        paste(missing.keys, collapse = ", "), "\n")
  }

  return( length(missing.keys)==0 )
}
...
config <- loadConfig()
stopifnot("Configuration file error." = configComplete(config))

rDir <- if( in.container() ) file.path("/", config$data.dir) else config$data.dir
if( !dir.exists(rDir) )
